...
tabs = st.tabs(["サマリー", "株価", "業績", "財務", "CF", "分析", "公式開示"])
...
with tabs[6]:
    ...
    st.caption("EDINETの公式提出書類を、投資判断上の重要度と内容まで確認します。")
        ...
        if st.button("直近45日の公式開示を取得", width="stretch"):
            ...
            if filings:
                for f in filings:
                    ...
                    f["投資判断上の意味"] = disclosure_meaning(f.get("書類名"))
                ...
                view = pd.DataFrame([
                    {
                        ...
                        "書類名": f.get("書類名", ""),
                        "投資判断上の意味": f.get("投資判断上の意味", disclosure_meaning(f.get("書類名"))),
                    }
                    for f in filings
                ])
