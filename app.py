...
def safe_pct_change(current, previous):
    a, b = n(current), n(previous)
    if a is None or b is None or b == 0:
        return None
    return a / b - 1

def add_income_analytics(inc):
    if inc is None or inc.empty:
        return pd.DataFrame()

    df = inc.copy().sort_values("年度").reset_index(drop=True)

    for col in ["売上高", "営業利益", "純利益", "EPS"]:
        if col in df.columns:
            df[f"{col}前年比"] = df[col].pct_change(fill_method=None)

    if "売上高" in df.columns:
        if "営業利益" in df.columns:
            df["営業利益率"] = df["営業利益"] / df["売上高"].replace(0, np.nan)
        if "純利益" in df.columns:
            df["純利益率"] = df["純利益"] / df["売上高"].replace(0, np.nan)

    return df

def latest_income_trend(df):
    result = {
        "売上高前年比": None,
        "営業利益前年比": None,
        "純利益前年比": None,
    ...
    for key in ["売上高前年比", "営業利益前年比", "純利益前年比", "EPS前年比", "営業利益率"]:
        if key in df.columns:
            result[key] = n(latest.get(key))

    if prev is not None and "営業利益率" in df.columns:
        cur_m = n(latest.get("営業利益率"))
        prev_m = n(prev.get("営業利益率"))
        if cur_m is not None and prev_m is not None:
            result["営業利益率前年差"] = cur_m - prev_m

    return result

def earnings_trend_comment(t):
    sales = t.get("売上高前年比")
    op = t.get("営業利益前年比")
    eps = t.get("EPS前年比")
    margin_delta = t.get("営業利益率前年差")

    positives, cautions = [], []

    if sales is not None:
        if sales >= 0.05:
            positives.append("売上高が前年比5%以上増加")
        elif sales < 0:
            cautions.append("売上高が前年割れ")

    if op is not None:
        if op >= 0.10:
            positives.append("営業利益が前年比10%以上増加")
        elif op < 0:
            cautions.append("営業利益が前年割れ")

    if eps is not None:
        if eps >= 0.10:
            positives.append("EPSが前年比10%以上増加")
        elif eps < 0:
            cautions.append("EPSが前年割れ")

    if margin_delta is not None:
        if margin_delta >= 0.01:
            positives.append("営業利益率が1pt以上改善")
        elif margin_delta <= -0.01:
            cautions.append("営業利益率が1pt以上悪化")

    if positives and not cautions:
        headline = "直近年度の業績トレンドは比較的良好です。"
    elif cautions and not positives:
        headline = "直近年度の業績トレンドには注意点があります。"
    elif positives and cautions:
        headline = "直近年度は強い項目と弱い項目が混在しています。"
    else:
        headline = "取得できた年度データだけでは明確なトレンド判定ができません。"

    return headline, positives, cautions

def format_pct(v, digits=1):
    v = n(v)
    return f"{v*100:.{digits}f}%" if v is not None else "取得不可"
...
# ホームページから ?code=8267 のように渡された場合は、その銘柄を自動分析
try:
    ...
    if query_code:
        ...
        if query_code.isdigit() and len(query_code) == 4:
            if st.session_state.stock_code != query_code or not st.session_state.analyzed:
                st.session_state.stock_code = query_code
...
tabs = st.tabs(["サマリー", "株価", "業績", "決算分析", "財務", "CF", "分析", "比較", "公式開示", "利用・法務"])
...
# サマリー
with tabs[0]:
    st.caption("Ver.12：株価・財務スコアに加えて、決算の増収増益・利益率・EPSトレンドを確認できます。")
...
# 決算分析
with tabs[3]:
    st.markdown("#### 決算トレンド分析")
    st.caption("取得できる年次実績から、増収・増益・利益率・EPSの変化を機械的に整理します。")
    ...
    inc_v12 = annual_table(fund, income_map_v12)
    analytics = add_income_analytics(inc_v12)
    trend = latest_income_trend(analytics)

    if analytics.empty:
        st.warning("決算トレンドを計算できる年次データがありません。")
    else:
        c1, c2 = st.columns(2)
        c1.metric("売上高 前年比", format_pct(trend["売上高前年比"]))
        c2.metric("営業利益 前年比", format_pct(trend["営業利益前年比"]))

        c1, c2 = st.columns(2)
        c1.metric("EPS 前年比", format_pct(trend["EPS前年比"]))
        c2.metric(
            "営業利益率",
            format_pct(trend["営業利益率"]),
            delta=(
                f"{trend['営業利益率前年差']*100:+.1f}pt"
                if trend["営業利益率前年差"] is not None else None
            ),
        )

        headline, positives, cautions = earnings_trend_comment(trend)
        st.markdown(f"**{headline}**")

        if positives:
            st.success("強い点： " + " / ".join(positives))
        if cautions:
            st.warning("注意点： " + " / ".join(cautions))

        # Mobile-friendly yearly table
        table = analytics.copy()
        keep = [
            "年度", "売上高前年比", "営業利益前年比", "純利益前年比",
            "EPS前年比", "営業利益率", "純利益率"
        ]
        table = table[[c for c in keep if c in table.columns]]
        for col in table.columns:
            if col != "年度":
                table[col] = table[col].apply(lambda x: format_pct(x))
        st.markdown("##### 年度別の変化")
        st.dataframe(table, hide_index=True, width="stretch")

        # Margin trend chart
        margin_cols = [c for c in ["営業利益率", "純利益率"] if c in analytics.columns]
        if margin_cols:
            fig = go.Figure()
            for col in margin_cols:
                fig.add_trace(go.Scatter(
                    x=analytics["年度"].astype(str),
                    y=analytics[col] * 100,
                    mode="lines+markers",
                    name=col,
                    hovertemplate=f"%{{x}}年<br>{col}: %{{y:.2f}}%<extra></extra>",
                ))
            fig.update_layout(
                title="利益率の推移",
                yaxis_title="%",
        ...
        # EPS trend chart
        if "EPS" in analytics.columns:
            fig = go.Figure()
            fig.add_trace(go.Bar(
                x=analytics["年度"].astype(str),
                y=analytics["EPS"],
                name="EPS",
                hovertemplate="%{x}年<br>EPS: %{y:,.2f}円<extra></extra>",
            ))
            fig.update_layout(
                title="EPS推移",
                yaxis_title="円",
    ...
    with st.expander("会社予想・進捗率について"):
        st.info(
            "現在のデータ取得方法では、会社が公表する通期予想を全銘柄で安定して取得できないため、"
            "予想値や進捗率を推測して表示していません。一次情報から安定取得できる仕組みを追加した段階で実装します。"
        )
